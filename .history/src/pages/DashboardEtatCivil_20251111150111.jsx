...
export default function DashboardEtatCivil() {
  ...
  // Données simulées : SEULEMENT ÉTAT CIVIL
  const stats = [
      ...
      <aside className="w-64 bg-white shadow-md h-screen fixed flex flex-col">
        <div className="p-6 border-b">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-600 rounded-lg flex items-center justify-center text-white text-xl">🏛️</div>
            <div>
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <SidebarMenu
            ...
            items={["Actes de naissance", "Actes de mariage", "Actes de décès", "Certificats"]}
          />

          <SidebarMenu
            icon={<FolderPlus className="w-5 h-5" />}
            label="Registres"
            items={["Registre des naissances", "Registre des mariages", "Registre des décès"]}
          />

          <SidebarMenu
            icon={<FileText className="w-5 h-5" />}
            label="Mes Documents"
            items={["Documents récents", "Mes brouillons", "Documents partagés"]}
          />

          <SidebarMenu
            icon={<Upload className="w-5 h-5" />}
            label="Validation"
            items={["En attente", "Validés", "Historique"]}
          />
        </nav>
      </aside>
      ...
      <div className="flex-1 ml-64">
        ...
        <main className="p-8">
          ...
          <div className="grid grid-cols-3 gap-6">
            {/* Documents récents - ÉTAT CIVIL UNIQUEMENT */}
    ...
    <tr key={i} className="border-b">
      ...
      <td className="py-4">
        <div className="flex gap-2">
          <Edit className={`w-4 h-4 cursor-pointer ${doc.status === 'Validé' || doc.status === 'Archivé' ? 'text-gray-400' : 'text-blue-600'}`} 
                title={doc.status === 'Validé' || doc.status === 'Archivé' ? 'Document verrouillé' : 'Modifier'} />
